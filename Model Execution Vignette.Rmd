...
In the Prime file:  
`File =` requires the path where the assessment folder is stored   
...
### Basic settings
JABBA provides various Graphic, Output, Saving options that can be specified in the prime file. If not specified, JABBA will automatically use the default settings as specified on top of [`JABBAv1.1.R`](https://github.com/jabbamodel/JABBA/blob/master/JABBAv1.1.R) code.  
...
JABBA makes it easy to run alternative scenarios in a loop. For this purpose, a unique name has to be assgined to each scenario. For example, a generic option to do this is for 10 alternative scenarios:

`Scenarios = c(paste0("Scenario",1:10))`    

but individual names can be specified as well, e.g. `Scenarios = c("Run_high_r","Run_medium_r","Run_low_r")`. JABBA automatically creates a folder for each scenario, including the `Input` and `output` subfolders.  Note that the type of model `_Schaefer`, `_Fox` and `_Pella` is automatically added to the name of scenario folder.   
...
This section is optional. For the SWO_SA example we use this section manipulate the input `cpue` and `se` data for scenarios 1-3. For sceanarios 2-3, we remove the BRA_LL1 series. For the base-case scenario 3, we use in addition seperate time series (time-blocks) for Japan (JP_LL1 & JP_LL2) and Spain (SP_LL1 & SP_LL2), which we merge back in one index for JP_1 and SP_LL1, respectively, for scenario 1-2. If `se` is provided, the same munipulations must be applied as for `cpue`.   
Again, always check if the manilupations were correctly applied.   
...
Most prior settings provide more than one option. For example, if K is meant to be specified as a lognormal prior set `K.dist = c("lnorm","range")[1]` or as range `K.dist = c("lnorm","range")[2]`. If the prior for K is specified as lognormal `K.prior = c(200000,1)` requires the untransformed mean K and the assumed CV, whereas if the prior for K is specified as range requires the assumed minum and maximum values, e.g. `K.prior = c(15000,1500000)`.          

The r prior provides an additional option by specifying a generic resiliance category *Very low, Low, Medium* or *High* as provided by [FishBase](www.FishBase.org).   
...
```{r Run Simulation, eval = FALSE, warning = F, message = F}
